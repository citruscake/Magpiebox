...
$(".delete_product_panel").click(function() {

	url = "/products/processAJAXQuery";
	product_id = $('#product_panel_product_id').html();
	
	jsonData = "{ \"command\" : \"delete\", \"product_id\" : \""+product_id+"\" }";

	$.post(url, {jsonData : JSON_data}, function(data) {
		console.log("Query executed successfully");
	}, "json");

	$('.close_product_panel').click();
	
});

$(".close_product_panel").click(function() {

	$('#page_overlay').fadeTo('fast', 0, "easeInCubic", function(){
		//animation complete
		$('#page_overlay').css('z-index',-90);
	});
	$('#product_editor').fadeTo('fast', 0, "easeInCubic", function(){
		//animation complete
		$('#product_editor').css("display", "none");
	});

});

$(".confirm_product_panel").click(function() {
	
	url = "/products/processAJAXQuery";
	product_id = $('#product_edit_product_id').val();
	category_id = $('#product_edit_category').val();
	name = $('#product_edit_name').val();
	description = $('#product_edit_description').val();
	quantity =  $('#product_edit_quantity').val();
	
	tags = "";
	$.each($('.tagit-label'), function(index, tag) {
	...
	JSON_data = "{ \"command\" : \"update\", \"product\" : {";
	JSON_data += "\"product_id\" : \""+product_id+"\", ";
